---
title: "Tidy workflows"
subtitle: ""
author: "Sina Rüeger"
date: "2018-10-04"
output:
  xaringan::moon_reader:
    css: ["default-fonts", "default", "my-theme.css"]
    lib_dir: libs
    nature:
      beforeInit: "https://platform.twitter.com/widgets.js"
      highlightStyle: github
      highlightLines: true
      titleSlideClass: [top, right]
      
---

<!-- updated: sys.date())
<!-- From here: https://slides.yihui.name/xaringan/ -->

```{r setup, include=FALSE}
options(htmltools.dir.version = FALSE)
library <- function(...) suppressPackageStartupMessages(base::library(...))
knitr::opts_chunk$set(
  tidy = TRUE
)
```

---
layout: true


---
class: left, middle

# About me

- Background in Data Analysis & Engineering

- PostDoc @ EPFL `r icon::ii_arrow_right_a()` Analysis of genetic data in infectious diseases at [Fellay Lab](https://fellay-lab.epfl.ch/).

- `r icon::fa_r_project(colour = "#88398a")`-Ladies Lausanne co-organiser


--

- `r icon::ii_heart()` Data analysis & Genetic data & Data visualisation

- `r icon::fa_question()` folder structure & `usethis` package

---
class: center, middle, inverse

# What is my everyday work?


---
class: left, middle


<img border="0" alt="" src="img/dailyworkflow/dailyworkflow.001.png" width="800">

.small[Adapted from Figure in [R4DS book](http://r4ds.had.co.nz/explore-intro.html)]

---
class: left, middle


<img border="0" alt="" src="img/dailyworkflow/dailyworkflow.002.png" width="800">

.small[Adapted from Figure in [R4DS book](http://r4ds.had.co.nz/explore-intro.html)]

---
class: center, middle

# What are the challenges?

- Making it easy for other colleagues to rerun my code (replicability of results!)
- Publishing code - making it easy for others to rerun with their data or to understand what we die (reproducibility of results!)
- Data comes in drops, will change over time, formats might change
- Iterating steps (cleaning, modelling, visualisation, reports)
- different places to compute things
- redundant code 

---
class: center, middle

# What we need

- enforcing tidyness in folders
- enforcing replicability (which will automatically lead to reproducibility)
- making things modular 
- minimising redundant computation


---
class: center, middle

# Package folder structure

given


---
class: center, middle

<a href="https://docs.google.com/presentation/d/1VK1hngMZSY3FT2SrDd4_AHiB28CHrsuSsaFr7r3SAL8/edit#slide=id.p">
<img border="0" alt="" src="img/heidibaya.png" width="800">
</a>

.small[Extract from presentation by Heidi Seibold @HeidiBaya on [*Tools for reproducibility in Statistics and Machine Learning*](https://docs.google.com/presentation/d/1VK1hngMZSY3FT2SrDd4_AHiB28CHrsuSsaFr7r3SAL8/edit#slide=id.p)]

---
class: center, middle, inverse

# What is out there?

---
class: left, top

# Wild west

---
class: left, top

# Wild west "pro"

---
class: left, top

adding a README file

---
class: left, top

# `make`


---
class: left, top

# other versions of make
- stu

Problem: other people need to learn this. so it need to be something intuitive. 

---
class: left, top

# Drake


---
<!---------
## Data journalism

<a href="https://www.srf.ch/static/srf-data/data/2018/federer/#/en">
<img border="0" alt="SRF" src="img/datajournalism.png" width="800">
</a>


[Source: SRF](https://www.srf.ch/static/srf-data/data/2018/federer/#/en)
----->

---
class: left, middle

# Diversity initiatives in R

- R Forwards

- R-Ladies

---
class: left, middle

# `r icon::fa_r_project(colour = "#88398a")`-Ladies

<img src="https://raw.githubusercontent.com/rladies/starter-kit/master/stickers/hex-logo-with-text.png" width="200">

PICTURE

- **Global** organisation.

- **Mission**: *To increase gender diversity in the R community* by encouraging, inspiring, and empowering underrepresented minorities.

- Founded in 2012 by [**Gabriela de Queiroz**](https://rladies.org/united-states-rladies/name/gabriela-de-queiroz/).

- Currently **125 R-Ladies meetup groups** in 40 countries.

- Find out more about **R-Ladies**: https://rladies.org/

.footnote[Recommended: listen to interview with [Gabriela de Queiroz](https://www.superdatascience.com/r-ladies-organization/)]

---
class: center, middle

<img src="https://raw.githubusercontent.com/rladies/Map-RLadies-Growing/master/rladies_growth.gif">

.footnote[[Source code](https://github.com/rladies/Map-RLadies-Growing) by [Daniela Vázquez](https://twitter.com/d4tagirl).]


---
class: left, middle

## Find other **chapters**: https://gqueiroz.shinyapps.io/rshinylady/

<img src="img/shinyapp.png">

---
class: left, middle

## Collaborations within R-Ladies: [meetupr package](https://github.com/rladies/meetupr)

<img src="img/meetupr.png">

---
class: left, middle

## Find speakers: [directory](https://rladies.org/directory/)

you can also add yourself!

<img src="img/rladiesdirectory.png">

---
class: left, middle

## [Community Slack](https://rladies-community-slack.herokuapp.com/)

- for people that identify as a woman or gender minority 
- CoC

Sign up here https://rladies-community-slack.herokuapp.com/

<img src="img/communityslack.png">

---
class: left, middle

## Join us in Lausanne

**R-Ladies Lausanne**: https://www.meetup.com/rladies-lausanne/

```
library(dplyr)
rladies_global
  %>% filter(city==“Lausanne”)
```

<img src="img/rladieslausanne.png">



---
class: inverse, center, middle

# Thank you!

Slides: [https://sinarueeger.github.io/20181004-geneve-rug/slides#1](https://sinarueeger.github.io/20181004-geneve-rug/slides#1)

Source code: [https://github.com/sinarueeger/20181004-geneve-rug/](https://github.com/sinarueeger/20181004-geneve-rug/)


`r icon::fa_twitter(colour = "white")`: [@sinarueeger](https://twitter.com/sinarueeger)
